...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_FILES['profile_picture'])) {
    ...
    $stmt = $conn->prepare("UPDATE business_users SET profile_picture = ? WHERE username = ?");
...
// Get profile picture
$stmt = $conn->prepare("SELECT profile_picture FROM business_users WHERE username = ?");
$stmt->bind_param("s", $viewed_user);
$stmt->execute();
            ...
            <?php foreach ($posts as $post): ?>
                <div class="tweet">
                    ...
                    <?php if ($username === $viewed_user): ?>
                        <form action="Bdelete_post.php" method="get" style="margin-top: 10px;">
