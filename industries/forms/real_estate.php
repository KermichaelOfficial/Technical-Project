...
// Fetch companies offering real estate investments
$companies = [];
$result = $conn->query("SELECT DISTINCT business_username FROM investment_options WHERE investment_type = 'real_estate'");
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $details = "Real Estate Investment Request\n\n";
    ...
    $details .= "Location: $property_location\n";
    $details .= "Property Value: $property_value\n";
    ...
    $filename = "Real_Estate_Request_" . time() . ".txt";
    $filepath = $dir . $filename;
    file_put_contents($filepath, $details);

    // Insert into database
    $stmt = $conn->prepare("INSERT INTO real_estate_requests (company_name, file_path, submitted_at) VALUES (?, ?, NOW())");
...
$mail->isHTML(true);
$mail->Subject = 'New Real Estate Investment Request';
$mail->Body    = "
    <h3>Hi $company_name,</h3>
    <p>You have received a new real estate investment request from $investor_name.</p>
...
$mail->Subject = 'Real Estate Investment Request Submitted';
$mail->Body    = "
    <h3>Hi $investor_name,</h3>
    <p>Your real estate investment request has been submitted successfully.</p>
...
$mail->send();
            ...
            echo "<script>alert('Real Estate Request Submitted! File saved and emails sent.'); window.location.href = '../../Uindex.php';</script>";
        } catch (Exception $e) {
...
<head>
    <meta charset="UTF-8">
    <title>Real Estate Investment Request</title>
...
<body>
<div class="container">
    <h2>Real Estate Investment Request Form</h2>
    <form method="POST">
        <!-- Investor Details -->
        ...
        <input type="text" name="property_location" id="property_location" required>

        <label for="property_value">Property Value:</label>
        <input type="number" name="property_value" id="property_value" required>
