...
// Handle file upload for articles, images, and videos
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_FILES['image']) && isset($_POST['image_title'])) {
        // Handle image upload
        ...
        if ($imageError === 0) {
            ...
            if (in_array(strtolower($imageExt), $allowedExts)) {
                ...
                $imageDestination = 'uploads/images/' . $newImageName;
                move_uploaded_file($imageTmpName, $imageDestination);

                // Insert into library_images table
                $stmt = $conn->prepare("INSERT INTO library_images (title, image_url, uploaded_by) VALUES (?, ?, ?)");
                $stmt->bind_param("sss", $imageTitle, $imageDestination, $username);
                $stmt->execute();
                $stmt->close();
            }
        }
    ...
    if (isset($_FILES['video']) && isset($_POST['video_title'])) {
        // Handle video upload
        ...
        if ($videoError === 0) {
            ...
            if (in_array(strtolower($videoExt), $allowedExts)) {
                $newVideoName = uniqid('', true) . '.' . $videoExt;
                $videoDestination = 'uploads/videos/' . $newVideoName;
                move_uploaded_file($videoTmpName, $videoDestination);

                // Insert into library_videos table
                $stmt = $conn->prepare("INSERT INTO library_videos (title, video_url, uploaded_by) VALUES (?, ?, ?)");
                $stmt->bind_param("sss", $videoTitle, $videoDestination, $username);
                $stmt->execute();
                $stmt->close();
            }
        }
    ...
    if (isset($_POST['article_title']) && isset($_POST['article_content'])) {
        ...
        // Insert into library_articles table
        $stmt = $conn->prepare("INSERT INTO library_articles (title, content, author) VALUES (?, ?, ?)");
...
<div class="container">
    ...
    <div class="main-content">
        <!-- Upload Section for Articles -->
        <div class="section">
            <h2>📚 Upload Investment Articles</h2>
            <form action="Alibrary.php" method="POST">
                <input type="text" name="article_title" placeholder="Article Title" required>
                <textarea name="article_content" placeholder="Article Content" required></textarea>
        ...
        <div class="section">
            <h2>📸 Upload Images</h2>
            <form action="Alibrary.php" method="POST" enctype="multipart/form-data">
